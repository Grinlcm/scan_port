...
def nmap_scan(ips, ports, url_path, out_port, out_url):
    scan_list = []
    print('Nmap starting.....')
    nm = nmap.PortScanner()
    if 'txt' in ips:
...
def main(ips, ports, url_path, out_port, out_url, str_type):
    if 'txt' in ips:
        ip_txt_handle(ips)
    port_list = []
    if 'txt' in ports:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 11:
        parser.print_help()
        exit()
    main(args.ip, args.ports, args.urlpath, args.outport, args.outurl)